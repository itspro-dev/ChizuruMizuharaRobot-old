...
async def hmm(_, message):
    ...
    if status == "ON" or status == "on" or status == "On":
        ...
        if not lol:
            ...
            return
        await lel.edit(
            f"Mizuhara AI Successfully Added For Users In The Chat {message.chat.id}"
        )
...
@asuna.on_message(
    filters.regex("mizuhara|Mizuhara|MIZUHARA")
    & ~filters.bot
    & ~filters.via_bot
    & ~filters.forwarded
    & ~filters.reply
...
__help__ = """
<b> Chatbot </b>
Mizuhara AI 3.0 IS THE ONLY AI SYSTEM WHICH CAN DETECT & REPLY UPTO 200 LANGUAGES
